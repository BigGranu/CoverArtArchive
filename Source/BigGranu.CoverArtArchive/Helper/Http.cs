...
namespace BigGranu.CoverArtArchive.Helper
{
    /// <summary>
    ///     Http Functions
    /// </summary>
            ...
            try
            {
                    ...
                    if (Count < 5)
                    {
                        switch (StatusCode)
                        {
                            ...
                            case HttpStatusCode.RedirectKeepVerb:
                                Count += 1;
                                return GetImageUrl(response.Headers["Location"]);
                        }
                    }
                }
#else
                ...
                using (var client = new System.Net.Http.HttpClient(handler))
                {
                    ...
                    if (Count < 5)
                    {
                        ...
                        {
                            case HttpStatusCode.Found:
                                return httpResponse.Headers.Location.ToString();
                            case HttpStatusCode.RedirectKeepVerb:
                                Count += 1;
                                return GetImageUrl(httpResponse.Headers.Location.ToString());
                        }
                    }
                }
#endif
